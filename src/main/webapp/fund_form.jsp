                ...
                $.ajax({
                    ...
                    success: function (data, status) {
                        ...
                        $.each(data, function (i, fund) {
                            ...
                            markup += '<tr style="cursor:hand" title="點兩下"><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td></tr>'.format(fund.fundId, fund.name, fund.desc, fund.fundnet.value, traderName, fund.stocks.length);
                        });
                        $("#fundTbody").empty();
